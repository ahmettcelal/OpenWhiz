 ...
 *   division by zero.
 * - Statistics (min, max) must be provided via `setStatistics` based on the training dataset.
 * 
 * **Unique Features:**
...
public:
     ...
     * @param size The number of features in the input.
     */
    owNormalizationLayer(size_t size) : m_size(size), m_min(1, size), m_max(1, size) {
